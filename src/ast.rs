use super::attribute::{Accent, ColumnAlign, LineThickness, Variant};
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Node::Fenced {
                ...
                write!(
                    f,
                    r#"<mrow><mo fence="true" form="prefix">{}</mo>{}<mo fence="true" form="postfix">{}</mo></mrow>"#,
                    open, content, close
                )
            }
            Node::StrechedOp(stretchy, op) => {
                write!(f, r#"<mo stretchy="{}">{}</mo>"#, stretchy, op)
            }
            Node::OtherOperator(op) => {
                if op == &"(" || op == &")" || op == &"[" || op == &"]" || op == &"{" || op == &"}"
                {
                    write!(f, r#"<mo stretchy="false">{}</mo>"#, op)
                } else {
                    write!(f, "<mo>{}</mo>", op)
                }
            }
            ...
            Node::Style(display, content) => match display {
                Some(DisplayStyle::Block) => {
                    write!(f, r#"<mstyle displaystyle="true">{}</mstyle>"#, content)
                }
                Some(DisplayStyle::Inline) => {
                    write!(f, r#"<mstyle displaystyle="false">{}</mstyle>"#, content)
                }
